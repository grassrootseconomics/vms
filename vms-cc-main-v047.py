...
import vmsutilsv043 as utils 
import colormaps as cmaps 
import datetime
from dateutil.relativedelta import relativedelta
from datetime import timedelta

STARTDATE = datetime.datetime(2017, 6, 1, 00, 00)
...
heatMode = True#False
heatOn = True #allows for the heatmap to turn on dynamically
ccMode = True
clientsModeB=False
seedingMode = False
...
tenxBtnRect = pygame.Surface(clearBtnSize )  # the size of your rect
...
'''
try:
    ccImage = pygame.image.load('cc.png')
    ncImage = pygame.image.load('nc.png')
...
NEIGHBORSEARCHSIZE = 60 #square area of nearest trade partners
...
def generateTradeBuddies(playerList):
    for dude in playerList:
        ...
        neighborIndex = len(neighborsList)-1
        
        while len(dude.tradeBuddies) < dude.numTradePartners and len(neighborsList) >0:
            if neighborIndex<0:
                neighborIndex=len(neighborsList)-1
            #print (len(neighborsList),neighborIndex)
            ...
            if len(neighborsList) <=0:
                ...
                neighborsList = generateTradeNeighbors(dude,currentList,currentSearchSize,dude.numTradePartners)
                neighborIndex=len(neighborsList)-1
            if currentSearchSize > 1000:
                print ("ERROR")
          
            #newTradeBuddy = neighborsList[neighborIndex]
            newTradeBuddy = random.choice(neighborsList)#neighborsList[neighborIndex]#
            #print (" nList: ", newTradeBuddy.tIndex,len(neighborsList), " dude: ", utils.traderTypeToString(dude.subType), " other: ", utils.traderTypeToString(newTradeBuddy.subType),newTradeBuddy.subType, "buddies: ",len(dude.tradeBuddies), bLOCALPRODUCER,pLOCALPRODUCER )
            neighborIndex+=1

            if newTradeBuddy.localSelling==False:
                neighborsList.remove(newTradeBuddy)
                currentList.remove(newTradeBuddy)
                #print ("remove1: ", newTradeBuddy.tIndex,len(neighborsList) )
                neighborIndex = len(neighborsList)-1
                continue
            if newTradeBuddy.localSelling==True and newTradeBuddy.subType==utils.RETAIL and bRETAIL < 1 and pRETAIL > 0:
                dude.tradeBuddies.append(newTradeBuddy)
                neighborsList.remove(newTradeBuddy)
                currentList.remove(newTradeBuddy)
                bRETAIL = bRETAIL +1
                #print ("remove2: ", newTradeBuddy.tIndex,len(neighborsList) )
                neighborIndex = len(neighborsList)-1
                continue
            elif newTradeBuddy.localSelling==True and newTradeBuddy.subType==utils.FOREIGNRETAIL and bFOREIGNRETAIL < 1 and pFOREIGNRETAIL > 0:
                dude.tradeBuddies.append(newTradeBuddy)
                neighborsList.remove(newTradeBuddy)
                currentList.remove(newTradeBuddy)
                bFOREIGNRETAIL = bFOREIGNRETAIL +1
                #print ("remove3: ", newTradeBuddy.tIndex,len(neighborsList) )
                neighborIndex = len(neighborsList)-1
                continue
            elif newTradeBuddy.localSelling==True and newTradeBuddy.subType==utils.LOCALSERVICE and bLOCALSERVICE < 1 and pLOCALSERVICE > 0:
                dude.tradeBuddies.append(newTradeBuddy)
                neighborsList.remove(newTradeBuddy)
                currentList.remove(newTradeBuddy)
                #print ("remove4: ", newTradeBuddy.tIndex,len(neighborsList) )
                bLOCALSERVICE = bLOCALSERVICE +1
                neighborIndex = len(neighborsList)-1
                continue
            elif newTradeBuddy.localSelling==True and newTradeBuddy.subType==utils.LOCALPRODUCER and bLOCALPRODUCER < 1 and pLOCALPRODUCER > 0:
                dude.tradeBuddies.append(newTradeBuddy)
                #print ("before-remove5: ", newTradeBuddy.tIndex,len(neighborsList) )
                neighborsList.remove(newTradeBuddy)
                currentList.remove(newTradeBuddy)
                bLOCALPRODUCER = bLOCALPRODUCER +1
                #print ("remove5: ", newTradeBuddy.tIndex,len(neighborsList) )
                neighborIndex = len(neighborsList)-1
                continue

            if newTradeBuddy.localSelling==True and len(dude.tradeBuddies)>=diversityLevelMax  :
                dude.tradeBuddies.append(newTradeBuddy)
                neighborsList.remove(newTradeBuddy)
                currentList.remove(newTradeBuddy)
                #print ("remove6: ", newTradeBuddy.tIndex,len(neighborsList) )
                neighborIndex = len(neighborsList)-1
            else:
                neighborIndex-=2
                #print("hoot",neighborIndex)

        #print "buddyList1"
        #for bu in dude.tradeBuddies:
        #    print ">>>",traderTypeToString(bu.subType)
...
#    print ("dude.rect: ",dude.rect)
    ...
    while passNeighbors == False:
        ...
        if searchRect.width >= WINDOWWIDTH*4:
            #print "b: no more found: search pass: ", searchPass, searchRect, len(neighbors)
            #print "buddies found: ", bRETAIL, bFOREIGNRETAIL, bLOCALPRODUCER, bLOCALSERVICE
            #print "numBuddies: ", numBuddies
            #print "numTradePartners: ", numTradePartners
            return neighbors
...
class Trade:
    def __init__(self,currency,amount,tradeType,source,dest):       
...
#            print aaa, coord
    ...
    def move(self):       
        ...
        #                print "mhoriz: ",mright, " mvert: ",mtop
        """
        if self.tradeType == EXTSERVICES: 
            if self.source.traderType == EXTERNALMARKET:
                print "moving EXTSERVICES money source = market"
        if self.tradeType == EXTSTOCK:
            if self.source.traderType == EXTERNALMARKET:
                print "moving EXTSTOCK money source = market"
        """
    ...
    def validate(self):
        ...
        if self.tradeType == utils.LOCALTRADESERVICES:
        ...
        elif self.tradeType == utils.DEPOSIT:
            ...
            if sourceMoney >= self.money:
                ...
                #print ("Error deposit")
                return False
        elif self.tradeType ==  utils.LOAN:
            self.dest.bankTrading == False
            if True:#self.source.savings >= self.money:
                return True
            else: 
                print ("Error Loan")
                return False
            ...
            if self.source.traderType == utils.BANK:
                if self.source.savings >= self.money:
                    return True
                else:
                    return False
            elif self.source.traderType != utils.BANK:
                if self.source.cc >= self.money:
                    return True
                elif self.source.cc > 0: 
                    self.money = self.source.cc
                else:
                    return False
            else: 
                #print "Error Clearing: source: ", utils.traderTypeToString(self.source.traderType), self.source.savings, self.source.cc, self.money
                return False
            ...
            if self.source.savings >= self.money:
                ...
                #print "Error Dividend"
                return False

        elif self.tradeType == utils.SAVINGS:

            if self.source.nc >= self.money:
                return True
            else: 
                #print "Error SAVINGS", utils.traderTypeToString(self.source.subType)
                return False
    ...
    def completeTrade(self):
        ...
        if self.tradeType == utils.LOCALTRADESERVICES:
            if self.dest.sellServices(selfMoney,self.currency) and self.source.purchaseServices(selfMoney,self.currency):
                ...
                return False
        elif self.tradeType ==  utils.DEPOSIT or self.tradeType ==  utils.SAVINGS or self.tradeType ==  utils.FEES:
            if self.source.makeDeposit(selfMoney,self.currency) and self.dest.acceptDeposit(selfMoney,self.currency):
                #self.source.adjustSize()
                return True
            else: 
                print ("Error Depost")
            ...
            if self.source.giveLoan(selfMoney,self.currency) and self.dest.acceptLoan(selfMoney,self.currency):
...
class Trader:
    ...
    def adjustSize(self):
        ...
        newMoney = self.nc+self.stock+self.cc+self.services-self.debt
...
#        print ("purchase final size: ", self.size, "final Money: ", self.money)
    ...
    def acceptLoan(self,amt,currency):
        #if self.savings >= amt:
        #    print ("error! amount = savings")
        #    return False
        

        interestDue = amt*theBank.profit#0.1
        #self.nc = self.nc+amt
        self.cc = self.cc+amt
        self.debt = self.debt + amt + interestDue# - self.savings
        #self.savings = 0
        self.lastLoanAmt = amt
        self.bankTrading = False 
        
        return True

    def giveLoan(self,amt,currency): #this should be the remaining amount after the persons amount has been deducted
        #ensure that the person has saved 1/4th the loan amount
        #if self.savings < amt:
            #print ("ERROR not enough money in bank")
            #b.loanTrading = False
            #return False

        self.debt = self.debt + amt 

        #newBalance = self.savings - amt
        #if newBalance <0:
        #    self.debt = self.debt + amt - self.savings
        #    self.savings = 0
            #print ("Bank in debt",self.debt)
        #else:
        #    self.savings = self.savings - amt
        self.loaned = self.loaned + amt
        return True
    ...
    def makeClearing(self,amt,currency): 
        global theBank
        #if self.nc < amt:
        ...
        if self.traderType == utils.BANK: #this is theBank
            ...
            if currency == utils.CC:
                print ("ERROR! bank in Clearing CC")
                return False

            if self.savings < amt:
                print ("ERROR! not enough savings for Clearing ")
                return False
            ...
            if currency == utils.NC:
                print ("ERROR! in Clearing NC")
                return False

            if self.cc < amt:
                print ("ERROR! not enough Clearing CC")
                return False
        ...
        print ("Error in MakeClearing")
        return False

    def acceptDeposit(self,amt,currency): 
        #this is the Bank
        leftOver = 0
        if currency == utils.NC:
            self.savings = self.savings+amt
            self.saved = self.saved + amt

        if currency == utils.CC: #puchase back the CC
            self.debt = self.debt-amt #negative debt = CC in stock

        self.deposits = self.deposits+amt
        return True



    def makeDeposit(self,amt,currency): 
        global theBank
        #if self.nc < amt:
        if currency == utils.NC:
            if self.nc < amt:
                self.bankTrading = False 
                print ("ERROR! in Deposit NC")
                return False
            ...
            if self.debt > 0:
                self.debt = self.debt - amt
                if self.debt < 0: #implies the savings amount more than cleared the debt
                    self.savings = self.savings + -1*self.debt
                    self.debt = 0
            else:    
                self.savings = self.savings+amt
        ...
        if currency == utils.CC:
            ...
            if self.debt > 0:
                self.debt = self.debt - amt
                if self.debt < 0: #implies the savings amount more than cleared the debt refund some CC
                    #print ("Error in Deposit")
                    self.cc = self.cc - self.debt
                    theBank.debt = theBank.debt - self.debt
                    #self.nc = self.nc + -1*self.debt #buying back the CC
                    #theBank.savings = theBank.savings + self.debt #reduce savings'
                    #theBank.debt = theBank.debt-amt #reduce debt
                    #print (theBank.savings, "BANK SAVINGS CHANGE")
                    self.debt = 0
            #else:    
                #print ("Error in Deposit2", self.debt)
                #self.nc = self.nc + amt #buying back the CC (theBank should reflect)
                #theBank.savings = theBank.savings - amt #reduce savings
                #theBank.debt = theBank.debt-amt #reduce debt
                #print (theBank.savings, "BANK SAVINGS CHANGE 2")
    ...
    def marketLogistics(self,theTraders):
        ...
        if exportMode and cycles - self.lastExportCycle >= self.waitToExportCycles:
            ...
            for b in currentTraders: #convert stock and services into cash
                ...
                if exportAmt>0:
                    tradebuddy=True
                    newExportTrade = Trade(utils.NC,exportAmt,tradeType,b.closestMarket,theTradeBuddy)
    ...
    #DIVIDEND
    def bankLogistics(self,theTraders):
        return
        global cycles

        #revenue = float(self.savings - self.debt) #20% of revenue is profit 80% costs

        requiredSavings = fractionalReserve*self.debt
        revenue = float(self.savings - requiredSavings) #20% of revenue is profit 80% costs

        costs = revenue *0.8 

        profits = revenue*0.2
        
        opCosts = costs*0.5 #Operationg Costs

        reserves = costs*0.3 #these add to equity 

        fProfit = costs*0.2#these goto GE extra Costs



        minDividend = 50.0
        if cycles - self.lastDividendCycle >= self.waitToDividendCycles and profits >= 1000:

            self.savings = self.savings - (fProfit + opCosts)
            self.nc = self.nc + (fProfit + opCosts) #profits held in another fund theBank.nc

            self.lastDividendCycle = cycles

            totalSavings = 0
            for b in theTraders: 
                totalSavings = float(totalSavings + b.savings)

            for b in theTraders: 
                #DEFAULTING TRADER
                if b.defaulting == True: #remove traders savings
                    if b.savings >= b.debt:
                        b.debt = 0
                        b.savings = b.savings - b.debt
                    elif b.savings < b.debt:
                        b.debt = b.debt - b.savings
                        b.savings = 0

                else:
                    pEquity = float(b.savings)/totalSavings
                    bDivi = pEquity*profits
                    #print (self.savings, self.debt, revenue, "| totsav:", totalSavings, b.savings, " | pEquity:", pEquity, b.debt, " | divi:", bDivi )

                    if bDivi >= minDividend:
                        newBankTrade = Trade(utils.NC,bDivi, utils.DIVIDEND,self,b) #source = bank, dest= other
                        b.addTrade(newBankTrade)                
                        theBank.addTrade(newBankTrade)
    ...
    def logistics(self):
        ...
        denomTSF = float(self.nc+self.cc+self.stock+self.services+1.0+self.savings)
        ...
        if self.nc+self.cc >= AVGTRADE and self.traderType == utils.GENERIC and cycles - self.lastNCTradeCycle >= self.waitToTradeCycles and numcBuddies >0 and selfLocalTrading and self.subType != utils.FOREIGNRETAIL:
            ...
            while len(cBuddies)>0:
                ...
                elif theTradeBuddy.subType == utils.LOCALSERVICE:
            ...
            if tradebuddy==True:
                ...
                if theTradeBuddy.subType != utils.FOREIGNRETAIL:
                    ...
                    if self.cc+self.nc >= tradeAmtOrig:

                        #Try a division of the price between CC and NC
                        if self.cc > 0  and theTradeBuddy.defaulting == False:
                            if theTradeBuddy.subType !=utils.RETAIL: #local producer and services
                                ...
                                if tradeCCAmt > self.cc:
                                    missingAmt = tradeCCAmt - self.cc
                ...
                if tradebuddy == True:
                    ...
                    if tradeNCAmt>=1:
                        ...
                        newTrade = Trade(utils.NC,tradeNCAmt,tradeType,self,theTradeBuddy)
                        self.addTrade(newTrade)
                        self.lastNCTradeCycle = cycles
                    
                    if tradeCCAmt>=1 and theTradeBuddy.defaulting == False:
                        newTradeCC = Trade(utils.CC,tradeCCAmt,tradeType,self,theTradeBuddy)
                        self.addTrade(newTradeCC)
        ...
        if importMode and self.importing == True and cycles - self.lastImportCycle >= self.waitToImportCycles and self.nc >= AVGTRADE and self.bankTrading == False and alreadyImporting == False:
            ...
            if True and self.stock>= self.STARTSTOCK and self.subType==utils.FOREIGNRETAIL and (myProfit > AVGTRADE) : #use excess money to buy services
                newImportTrade = Trade(utils.NC,myProfit,utils.EXTSERVICES,self,self.closestMarket)
            ...
            if importAmt > 0:
                #print (importAmt)
                newImportTrade = Trade(utils.NC,importAmt,utils.EXTSTOCK,self,self.closestMarket)
                self.addTrade(newImportTrade)
                self.lastImportCycle = cycles
                self.bankTrading == True
        ...
        #ISSUE
        #CONVERT TO CC #if profits are up - convert NC to CC and use it to buy stuff
        if self.coop and clearingMode and ccVault > 0 and cycles - self.lastClearingCycle >= self.waitToClearCycles  and self.nc>= AVGTRADE and alreadyBanking==False and self.subType != utils.FOREIGNRETAIL:

            #myProfit = self.cc-(self.STARTNC-self.nc)-(self.STARTSTOCK-self.stock)-self.debt
            myProfit = self.cc-(self.STARTNC-self.nc)
            if myProfit >=0 and self.cc <= AVGTRADE:
                if self.nc < myProfit:
                    myProfit = self.nc
                    
                moreCC = ccXng*myProfit

                if ccVault < moreCC:
                    moreCC = ccVault
                self.cc = moreCC+self.cc
                self.nc = self.nc-myProfit
...
#    playerRect.topleft = (WINDOWWIDTH / 2, WINDOWHEIGHT - 50)
    ...
    while runSim: # the game loop runs while the game part is playing
                ...
                for s in legendSprites:
                    if s.rect.collidepoint(event.pos):
                        ...
                        if s.legendType == utils.EXTERNALMARKET:
                            break
                        elif s.legendType == utils.NC:
                            NCHEAT = not(NCHEAT)
                            for g in traders:
                                for h in g.trades:
                                    if h.currency==utils.NC and h.dest.traderType !=  utils.EXTERNALMARKET and h.source.traderType !=  utils.EXTERNALMARKET:
                                        h.heating = NCHEAT
                        elif s.legendType == utils.CC:
                            CCHEAT = not(CCHEAT)
                            for g in traders:
                                for h in g.trades:
                                    if h.currency==utils.CC and h.dest.traderType !=  utils.EXTERNALMARKET and h.source.traderType !=  utils.EXTERNALMARKET:
                                        h.heating = CCHEAT
        ...
        if displayPlots == True :
            ...
            drawText('CC', fontSysInfo, background, legendOffsetX, legendOffsetY-30)
...
#legendSprites.add(legendDude(ncImage, (legendOffsetX+65, legendOffsetY-25),utils.EXTERNALMARKET))
            ...
            if clickedTrader >= 0 and len(traders)>0 and clickedTrader < len(traders):
                ...
                cOffseting = cOffseting + cOffInc
                drawText('Current %sNC, %scc, Stock %s, Services %s (profit %s)' % (int(cTraderSelf.nc),int(cTraderSelf.cc),int(cTraderSelf.stock),int(cTraderSelf.services),int(cTraderSelf.cc-(cTraderSelf.STARTNC-cTraderSelf.nc)-(cTraderSelf.STARTSTOCK-cTraderSelf.stock)-cTraderSelf.debt)), 
                         fontTrInfo, background, cOffsettingX, cOffseting)
