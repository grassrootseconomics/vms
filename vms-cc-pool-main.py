"""vms-cc-pools-main-v050: Village Market Simulator with Commitment Pools.
Original Author: William O. Ruddick
Contact: will@grassecon.org
Website: grassecon.org
Original Date: May-18-2018
New release Date: Aug-22-2024
Code: Draft - pre-release v050

This simulator takes an agent based approach to simulating basic economic intereactions with multiple vouchers.

A model of a polycentric resource coordination system including commitment pools, implemented for sustainable development, is presented here and used as a basis for an Agent Based Model (ABM). The ABM presents a simplified approach to understanding the mechanisms, impacts and limitations of Community Currencies/ Community Asset Vouchers and Commitment Poolson regional markets. Findings from the model must be validated by survey data. Preliminary findings suggest that CCs can stabilize and increase trade in local markets during erratic or seasonal exogenous market conditions. Furthermore, the effects  on local markets are found to be highly dependent on the size of existing local production and service sectors.
"""
...
coolingAmtOrig = 6#13#amount of cooling for heatmap
heatingAmtOrig = 12#19#amount of heating for heatmap
coolingFREQOrig = 8#2#for heatmap number of cycles to wait to cool
...
ncStartAmt = 0#10000  # Amount of National Currency allocated at start
...
NUMTRADEPARTNERS = 8 # total number of trading partners
if MAXTRADERS <=5:
...
def cyclesToDate():
    ...
    STARTDATE = datetime.datetime(2024, 8, 7, 00, 00) #1st trading on blockchain of a tomato in Kenya
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                if event.key == K_ESCAPE:
                    terminate()
                elif event.key == pygame.K_q: # pressing escape quits
                    print("q down")
                    terminate()
...
def generateTradeBuddies(playerList):
    for dude in playerList:
        if dude.subType==utils.FOREIGNRETAIL or dude.subType==utils.MYCOAGENT:
            continue
        ...
        while len(dude.tradeBuddies) < dude.numTradePartners and currentSearchSize < MAXNEIGHBORSEARCHSIZE:
            ...
            if len(neighborsList) > 0:
                ...
                if newTradeBuddy.localSelling==True and newTradeBuddy.subType!=utils.MYCOAGENT:
...
#        print ("search pass: ", searchPass, searchRect)
    for p in players:
        if p.subType!=utils.MYCOAGENT and p.rect.colliderect(searchRect)==1 and p.rect.colliderect(oldSearchRect)!=1:
            neighbors.append(p)
...
class Trade:
    def __init__(self,currency,amount,tradeType,source,dest,token,path):
...
#            print aaa, coord
            ...
            if utils.isCC(self.currency):#==utils.CC:
                ...
                if self.currency == clickedToken:
                    ...
                    if hasattr(self.token, 'spriteImg'):
                        toImage = self.token.spriteImg
                    else:
                        print("Error on toImage",toImage, " source: ", utils.traderTypeToString(self.source.subType), " dest: ", utils.traderTypeToString(self.dest.subType))
    ...
    def validate(self):
        ...
        if(self.source.subType==utils.MYCOAGENT or self.dest.subType==utils.MYCOAGENT): 
            #return True

            s_voucher = next((voucher for voucher in self.source.cc if voucher.token.tokenID == self.path[0]),None)
            #print("from voucher: ", fvoucher.token.tokenID, fvoucher.balance)
            if(s_voucher == None):
                return False
            elif(s_voucher and s_voucher.balance >= self.money):
                return True
            else:
                self.money = s_voucher.balance
                if(False):#debug
                    print("!!!!!!!!!!!!!!!!Failed on pool validation!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    if(self.dest.subType==utils.MYCOAGENT):
                        print("dest total in pool: ", self.dest.getCCValue(), "source balance: ", s_voucher.balance, " trade size: ", self.money)
                    if(self.source.subType==utils.MYCOAGENT):
                        print("source total in pool: ", self.source.getCCValue(), "source balance: ", s_voucher.balance, " trade size: ", self.money)
    ...
    def completeTrade(self):
        ...
        if(self.source.subType==utils.MYCOAGENT or self.dest.subType==utils.MYCOAGENT): 
            ...
            if(s_voucher == None):
                return False
            s_voucher.balance -= self.money
            #print("source: (", self.source.tIndex, " ", utils.traderTypeToString(self.source.subType), ") new balance: ", s_voucher.balance, " old bal: ", s_voucher.balance+self.money, " total cc:", self.source.getCCValue())
            d_voucher = next((voucher for voucher in self.dest.cc if voucher.token.tokenID == self.path[0]),None)
            if(d_voucher == None):
                return False

            d_voucher.balance += self.money
            #print("dest: (", self.dest.tIndex,  " ", utils.traderTypeToString(self.dest.subType), ") new balance: ", d_voucher.balance, " old bal: ",d_voucher.balance-self.money, " total cc:", self.dest.getCCValue())


            if(False): #debug
                if(self.dest.subType==utils.MYCOAGENT):
                    print("dest total in pool: ", self.dest.getCCValue())
                if(self.source.subType==utils.MYCOAGENT):
                    print("source total in pool: ", self.source.getCCValue())
                ...
                for t in traders:
                    ...
                    for cc in t.cc:
                        print("   val: ", cc.token.tokenID, cc.balance)
            ...
            if(len(self.path)>1):
                ...
                next_voucher = next((voucher for voucher in self.dest.cc if voucher.token.tokenID == self.path[1]),None)
                if(next_voucher == None):
                    return False
                #now send another Trade send path[1]
                self.path.pop(0)
                if(len(self.path)>=2):
...
class Trader:
    ...
    def addTradeList(self,tt):
        if displayPlots:
            ...
            max_text_lines = 18
            if(self.subType==utils.MYCOAGENT):
                max_text_lines = 10
            if len(self.tradeInfo) > max_text_lines:
                del self.tradeInfo[max_text_lines:]

    def convertAllCC(self,destToken):
        print("depricated 123a")
        return
        #traderRem = self
        deleteDest = False
        removeList = []
        ...
        for wtkn in self.cc:
            ...
            if wtkn.token.tokenID != destToken.tokenID and wtkn.token.tokenID != reserveToken.tokenID and wtkn.balance > 0 and wtkn.getConnector != None and destToken.connector != None:
                wbal = wtkn.balance
                if wbal < 0:
    ...
    def setupMycoAgent(self):

        print ("mycoAgent Created")
        self.subType=utils.MYCOAGENT

        self.STARTCC=0
        self.STARTNC=0
        self.STARTNC=0#random.gauss(ncStartAmt,ncStartStd)
        if self.STARTNC<=0:
            self.STARTNC = 0
        self.STARTSTOCK=0
        self.STARTSERVICES=servicesStartAmt#random.gauss(servicesStartAmt,servicesStartStd)
    ...
    def create_swap_pool(self,click_pos,num_c):
        ...
        if(len(closest_agents) >=1):
            ...
            print("No agents with vouchers found!")
...
#        print ("purchase final size: ", self.size, "final Money: ", self.money)
    ...
    def drawSelf(self,theSurface):
        ...
        if(self.subType==utils.MYCOAGENT): #draw connections
            for cc in self.cc:
                cc_id = cc.token.tokenID
                for agent in traders:
                    if(agent.preferedToken != None):
                        if(agent.preferedToken.tokenID == cc_id):
                            rect = agent.rect
                            center = rect.center
                            pygame.draw.line(theSurface, WHITE, self.rect.center, center, width=1)
    ...
    def marketLogistics(self,theTraders):
        ...
        if exportMode and cycles - self.lastExportCycle >= self.waitToExportCycles:
            ...
            for b in currentTraders: #convert stock and services into cash
                ...
                if exportAmt>0:
                    tradebuddy=True
                    newExportTrade = Trade(utils.NC,exportAmt,tradeType,b.closestMarket,theTradeBuddy,None,None)
    ...
    def logistics(self):
        ...
        numeratorTSF = float(self.STARTNC+self.STARTSTOCK+self.STARTSERVICES+1.0)#+self.STARTSTOCK+self.STARTSERVICES+1.0)
        ...
        if self.waitToTradeCycles < DAILYCYCLES/6:
            #print("Too fast", self.waitToTradeCycles)
            self.waitToTradeCycles = DAILYCYCLES/6

        if self.waitToTradeCycles > DAILYCYCLES*6:
            #print("Too slow", self.waitToTradeCycles)
            self.waitToTradeCycles = DAILYCYCLES*6
        ...
        #if not(self.nc+self.getCCValue() >= AVGTRADE and self.traderType == utils.GENERIC and cycles - self.lastNCTradeCycle >= self.waitToTradeCycles and cycles - self.lastCCTradeCycle >= self.waitToTradeCycles and numcBuddies >0 and selfLocalTrading and self.subType != utils.FOREIGNRETAIL and self.subType != utils.MYCOAGENT):
            #print(self.nc+self.getCCValue() >= AVGTRADE, self.traderType == utils.GENERIC, cycles - self.lastNCTradeCycle >= self.waitToTradeCycles, cycles - self.lastCCTradeCycle >= self.waitToTradeCycles,  numcBuddies >0, selfLocalTrading, self.subType != utils.FOREIGNRETAIL, self.subType != utils.MYCOAGENT)

            
        #print("outside")
        #TRADE with other TRADERS
        if self.nc+self.getCCValue() >= AVGTRADE and self.traderType == utils.GENERIC and cycles - self.lastNCTradeCycle >= self.waitToTradeCycles and cycles - self.lastCCTradeCycle >= self.waitToTradeCycles and numcBuddies >0 and selfLocalTrading and self.subType != utils.FOREIGNRETAIL and self.subType != utils.MYCOAGENT:
            #print("inside")
            ...
            random.shuffle(self.tradeBuddies)
            ...
            while len(cBuddies)>0:
                ...
                if theTradeBuddy.localSelling == False or selfLocalTrading == False:
                    ...
                    #print("!no!3 self: ", utils.traderTypeToString(self.subType), " tradeBuddy: ", utils.traderTypeToString(theTradeBuddy.subType), self.tradeBuddies)
                    tradebuddy = False
                    cBuddies.remove(theTradeBuddy)
            ...
            if tradebuddy==True:
                ...
                missingAmt = 0
                if(self.nc < tradeNCAmt):
                    missingAmt = tradeAmtOrig-self.nc
                    tradeNCAmt = self.nc
                tradeCCAmt = missingAmt

                if(tradeNCAmt >0):
                    newTrade = Trade(utils.NC,tradeNCAmt,tradeType,self,theTradeBuddy,None,None)
                    self.addTrade(newTrade)
                    self.lastNCTradeCycle = cycles
                    if(False):
                        print(utils.traderTypeToString(self.subType), "Made a NC Trade! to: ", utils.traderTypeToString(theTradeBuddy.subType), " amt: ", tradeNCAmt, " waiting: ", self.waitToTradeCycles, " seepdFactor: ", tradeSpeedFactor, " NC: ", self.nc)
                        totalCC = 0
                        totalNC = 0
                        for t in traders:
                            vale = t.getCCValue()
                            valenc = t.nc
                            print("   -- ", utils.traderTypeToString(t.subType), " amtcc: ", vale, "  NC val: ", t.nc)
                            for cc in t.cc:
                                print("       cc val: ", cc.token.tokenID, cc.balance)
                            totalCC += t.getCCValue()
                            totalNC += valenc

                        print("<><><>total CC: ", totalCC, "<><> total NC: ", totalNC)
                        ...
                        if(self.preferedToken != None and theTradeBuddy.preferedToken.tokenID != None):
                            if(self.preferedToken.tokenID in G and theTradeBuddy.preferedToken.tokenID in G):
                                ...
                                if feasible_paths:
                                    ...
                                    #print("Best exchange route:", best_path)
        ...
        if importMode and self.importing == True and cycles - self.lastImportCycle >= self.waitToImportCycles and self.nc >= AVGTRADE and self.bankTrading == False and alreadyImporting == False:
            ...
            if True and self.stock>= self.STARTSTOCK and self.subType==utils.FOREIGNRETAIL and (myProfit > AVGTRADE) : #use excess money to buy services
                newImportTrade = Trade(utils.NC,myProfit,utils.EXTSERVICES,self,self.closestMarket,None,None)
            ...
            if importAmt > 0:
                #print (importAmt)
                newImportTrade = Trade(utils.NC,importAmt,utils.EXTSTOCK,self,self.closestMarket,None,None)
                self.addTrade(newImportTrade)
                self.lastImportCycle = cycles
        ...
        if clearingMode and self.getCCValue()>0 and cycles - self.lastClearingCycle >= self.waitToClearCycles  and  alreadyBanking==False and self.subType != utils.FOREIGNRETAIL and self.clearingPercentage >0:
            ...
            tknToClear = None
            clearingTknAmt = 0
            if self.preferedToken is not None and self.preferedToken.connector is not None:
        ...
        #ANYONE Create a Voucher
        if swapMode and self.ownToken == False and self.subType != utils.MYCOAGENT:

            moreCC = self.STARTSTOCK+self.STARTSERVICES
            ...
            newToken = Token(lastTokenID+1,moreCC,None,1,True,newImage) #this is 1:1 to nc no connector
            ...
            print(self.cc[0].token.tokenID, " supply:", self.cc[0].token.supply)
...
legendSprites.add(legendDude(mycoImage, (legendOffsetX, legendOffsetY-50),utils.MYCOAGENT))
    ...
    while runSim: # the game loop runs while the game part is playing
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYUP:
                if event.key == K_ESCAPE:
                    ...
                    swapMode = not swapMode #exportTrader start
                    #importMode = not importMode
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if tenxBntRectal.collidepoint(event.pos):
                    ...
                    while d < newDudes:
                        ...
                        elif slegendType == utils.MYCOAGENT:
                            tn.setupMycoAgent()
                            pMYCOAGENT = pMYCOAGENT + 1
                        ...
                        #MAXTRADERS = 100
                        MAXTRADERS = pRETAIL + pFOREIGNRETAIL + pLOCALSERVICE + pLOCALPRODUCER + pEXPORTSERVICE + pMYCOAGENT
                ...
                for s in legendSprites:
                    if s.rect.collidepoint(event.pos) and s.legendType == utils.EXCHANGE:
                        ...
                        if s.legendType == utils.EXTERNALMARKET:
                            ...
                            pFOREIGNRETAIL = pFOREIGNRETAIL + 1
                        elif s.legendType == utils.MYCOAGENT:
                            tn.setupMycoAgent()
                            print("MYCO POOL PICK")
                            pMYCOAGENT += 1
                ...
                if traders[clickedTrader].rect.left < INFOWIDTH:
                    ...
                    traderRem.convertAllCC(reserveToken)
        ...
        for b in traders:
            ...
            for trade in b.trades:
                ...
                if trade.hasArrived():
                    #print "arrived"
                    if trade.validate(): #validate swap
...
#legendSprites.add(legendDude(ncImage, (legendOffsetX+65, legendOffsetY-25),utils.EXTERNALMARKET))
            ...
            drawText('Pool', fontSysInfo, background, legendOffsetX-5, legendOffsetY-30)
            ...
            if clickedTrader >= 0 and len(traders)>0 and clickedTrader < len(traders):
                ...
                if len(cTraderSelf.cc)>0:
                    tknPerf = cTraderSelf.preferedToken
                    tknPerfId = 'None'
                    if tknPerf != None:
                        tknPerfId = cTraderSelf.preferedToken.tokenID
                    drawText('Tokens pref: %s' % (tknPerfId), fontTrInfo, background, cOffsettingX, cOffseting)
...
#        if monthlyLocalTradedListNCLength >= 30:
            ...
            #print ("stuff: ", clearingMode,",",currentMonthNum, ",",sumAll, ",",avgAll,",",sumNC , ",",sumCC, ",", sumExport,",",avgNC , ",",avgCC, ",", avgExport ,",",IMPORTMULT,",",today)
